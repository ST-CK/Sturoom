...
export type PresenceMetrics = {
  todayMin: number; avg7Min: number; weekMin: number; trend: number[];
};
export type StudyMetrics = {
  totalH: number; totalM: number; avgPerSessionMin: number; sessions: number; trend: number[];
};
export type VolumeMetrics = {
  problems: number; videosMin: number; weekDiffProblems: number; weekDiffVideos: number;
};
