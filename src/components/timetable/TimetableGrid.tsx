...
export default function TimetableGrid({ stage, entries, onClickEmpty, onClickEntry, onDeleteEntry }: GridProps) {
  const { hours, containerHeight, startHour } = useMemo(() => {
    if (stage === 'university') {
      const hrs = Array.from({ length: END_HOUR_UNI - START_HOUR_UNI + 1 }, (_, i) => START_HOUR_UNI + i);
      const height = (END_HOUR_UNI - START_HOUR_UNI) * 60 * PX_PER_MIN;
      return { hours: hrs, containerHeight: height, startHour: START_HOUR_UNI };
    }
    ...
    <div className="w-full overflow-x-auto">
      <div className="min-w-[900px]">
        ...
        <div className="max-h-[70vh] overflow-y-auto">
          {/* 요일 헤더 - sticky */}
          <div className="grid grid-cols-[80px_repeat(7,1fr)] text-sm font-medium sticky top-0 z-10 bg-white/90 backdrop-blur">
            <div className="py-2" />
            {DAYS.map((d, idx) => (
              <div key={idx} className="py-2 text-center text-gray-700">{d}</div>
            ))}
          </div>
...
function DayColumn({ day, entries, containerHeight, startHour, onClickEmpty, onClickEntry, onDeleteEntry }: {
  day: number;
  entries: TimetableEntry[];
    ...
    >
      ...
      {entries.map((e) => {
        ...
        return (
          ...
          >
            {/* ✅ 시간칩 제거, 제목/장소만 */}
            <div className="flex h-full flex-col justify-center gap-1 p-2 text-xs" style={{ color: text }}>
              <div className="font-semibold leading-tight truncate">{e.title}</div>
              {e.location && <div className="opacity-80 truncate">{e.location}</div>}
            </div>

            <button
              className="absolute bottom-1.5 right-1.5 text-[10px] px-1.5 py-0.5 rounded-lg bg-white/80 hover:bg-white border border-black/5"
